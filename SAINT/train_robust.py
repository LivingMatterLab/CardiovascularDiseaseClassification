...
from data_openml import data_prep_pickle,task_dset_ids,DataSetCatCon
import argparse
...
parser.add_argument('--dset_pkl', required = True, type = str)
...
parser.add_argument('--savemodelroot', default='./bestmodels_saint', type=str)
...
# modelsave_path = os.path.join(os.getcwd(),opt.savemodelroot,opt.task,str(opt.dset_pkl),opt.run_name)
modelsave_path = os.path.join(os.getcwd(),opt.savemodelroot,opt.task,str(os.path.basename(opt.dset_pkl)),opt.run_name)
...
# cat_dims, cat_idxs, con_idxs, X_train, y_train, X_valid, y_valid, X_test, y_test, train_mean, train_std = data_prep_openml(opt.dset_pkl, opt.dset_seed,opt.task, datasplit=[.65, .15, .2])
set_index = re.findall(r'\d+', opt.dset_pkl)[0]
cat_dims, cat_idxs, con_idxs, X_train, y_train, X_valid, y_valid, X_test, y_test, train_mean, train_std = data_prep_pickle(opt.dset_pkl, set_index, opt.dset_seed, opt.task)
...
###################
# Save model properties for reconsturction from state later
print("Saving model properties...")
with open('%s/properties.pkl' % (modelsave_path), 'wb') as f:
    pickle.dump([(tuple(cat_dims), len(con_idxs), opt.embedding_size, 1, opt.transformer_depth, 
                 opt.attention_heads, opt.attention_dropout, opt.ff_dropout, (4, 2), opt.cont_embeddings,
                 opt.attentiontype, opt.final_mlp_style, y_dim), 
    ...
    if epoch%5==0:
            ...
            with torch.no_grad():
                if opt.task in ['binary','multiclass']:
                    ## Modifcation: also report training set metrics ##
