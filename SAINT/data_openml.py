...
# Adapted the function data_prep_openml to take in a pickled
# dataset structure ds_pkl rather than an openml dataset ID
def data_prep_pickle(ds_pkl, index, seed, task):
    ...
    X, y, train_indices, valid_indices, test_indices = dataset
    categorical_indicator = np.full((len(X.columns),), False)
    ...
    # The data is already shuffled, so we can instead perform an explicit split
    # m_total = X.shape[0] # Number of all examples
    # train_indices = list(range(0, int(datasplit[0] * m_total)))
    # valid_indices = list(range(int(datasplit[0] * m_total), int((datasplit[0] + datasplit[1]) * m_total)))
    # test_indices = list(range(int((datasplit[0] + datasplit[1]) * m_total), m_total))
    ...
    print("Saving SAINT train-val-test sets...")
    with open('SAINT/saint_datasets/train_val_test' + str(index) + '.pkl', 'wb') as f:
